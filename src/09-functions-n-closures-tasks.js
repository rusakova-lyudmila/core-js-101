...
function getPolynom(...arr) {
  ...
  return (x) => arr.reduce((sum, item, ind) => {
    const powX = arr.length - 1 - ind;
    return sum + item * x ** powX;
  }, 0);
}
...
function logger(func, logFunc) {
  return (...arg) => {
    const outputLogStr = `${func.name}(${arg.map((item) => JSON.stringify(item)).join(',')})`;

    logFunc(`${outputLogStr} starts`);
    const res = func(...arg);
    logFunc(`${outputLogStr} ends`);
